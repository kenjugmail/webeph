...
(function () {
  ...
  const FRAG = `
    ...
    void main(){
      ...
      float d = length(uv);
      if(after){
        // smooth, on-palette glow (violet→teal→brass)
        vec3 col = pal(f + d*0.4 + t*0.2,
          vec3(0.18,0.16,0.28), vec3(0.55,0.5,0.6),
          vec3(1.0,0.9,0.7), vec3(0.1,0.35,0.62));
        col += vec3(0.18,0.34,0.55)*pow(max(0.,1.0-d),3.0);   // teal-violet core bloom
  ...
  window.mountReview = mountReview;
})();
