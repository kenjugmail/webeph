...
create table if not exists public.research_reviews (
  ...
  recommendation text not null default 'not_applicable'
...
-- Publish the reviewed immutable version, its approved file register, article
-- status, identifier, and audit event in one transaction. Storage copies happen
-- before this call and are removed by the editor client if this transaction fails.
drop function if exists public.publish_research_submission_with_files(uuid, text, jsonb, text, text, text);
...
begin
  ...
  loop
    ...
    if position(expected_prefix in coalesce(file_row->>'storage_path', '')) <> 1
       or position('..' in coalesce(file_row->>'storage_path', '')) > 0 then raise exception 'Published file path does not match the immutable version'; end if;
    if nullif(trim(file_row->>'original_filename'), '') is null
       or (file_row->>'original_filename') ~ '[/\\]' then raise exception 'Published file has an unsafe filename'; end if;
    if coalesce((file_row->>'byte_size')::bigint, 0) < 1
       or (file_row->>'byte_size')::bigint > 52428800 then raise exception 'Published file exceeds the 50 MiB limit or is empty'; end if;
    if coalesce(file_row->>'sha256', '') !~ '^[0-9a-f]{64}$' then raise exception 'Published file requires a lowercase SHA-256 hash'; end if;
    if nullif(trim(file_row->>'mime_type'), '') is null
       or (file_row->>'mime_type') ~ E'[\\n\\r]' then raise exception 'Published file has an invalid MIME type'; end if;
...
revoke execute on function public.publish_research_submission(uuid, text, text, text, text) from public, anon, authenticated;
...
-- private checklist state, and all other private workflow fields. Public funding,
-- conflict, ethics, AI, licensing, and accountability declarations remain visible.
