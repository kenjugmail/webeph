...
assert.equal(release.assets.length, 2, "reviewed Mac alpha and Windows public beta are available");
...
const windows = release.assets.find((asset) => asset.platform === "windows-x64");
assert.equal(windows.signing, "unsigned");
assert.equal(windows.beta, true);
assert.equal(windows.sizeBytes, 16733067);
assert.equal(windows.sha256, "1cc7c3c767b6fe30db3ed1ac5d334f369d4ca268b3481227af44eadfd15634e1");
assert.equal(windows.url, "https://github.com/kenjugmail/hearthrail-releases/releases/download/v0.1.0-alpha.1-windows-beta/Hearthrail_0.1.0-alpha.1_x64-setup.exe");
assert.ok(staticCard.includes(`href="${windows.url}"`));
assert.ok(staticCard.includes(windows.sha256));
assert.match(staticCard, /Unsigned beta \(Windows\)/);
assert.doesNotMatch(page, /Windows is (?:not available|unavailable)/);
...
assert.match(home, /href="\/hearthrail"[\s\S]{0,700}Public preview/);
